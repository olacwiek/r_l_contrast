...
Logistic regression coefficients are notoriously hard to interpret, s they are reported on the log odds scale.
    ...
    !(.save_models & file.exists("./models/web_regressions_L1_models.rds")) )
{
  ...
  brms::hypothesis(b_L1_full, c("Orderl_first = 0",                                      # p=0.58
                                "SexM = 0",                                              # p=0.82
                                "Age = 0",                                               # p=0.99
                                "r_l_distinction_L1_fdistinct = 0",                      # p=0.70
  ...
  if( FALSE ) # not really needed to run: what matters is the final table...
  {
    ...
    hdi(b_otexperiments_1_int); # [-0.94,  1.81]
    rope(b_otexperiments_1_int, , ci=0.95, ci_method="HDI"); # 17.48 %
  ...
  # check if Area and Family are needed:
  b_null_wr_nofa <- brm(Match ~ 1 + 
                     (1 | Language),
                   data = web_wr,
                   family=bernoulli(link='logit'),
                   save_pars=save_pars(all=TRUE), # needed for Bayes factors
                   sample_prior=TRUE,             # needed for hypotheses tests
                   seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=13));
  summary(b_null_wr_nofa); mcmc_plot(b_null_wr_nofa, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_null_wr_nofa, variable="^b_", regex=TRUE); # very decent
  b_null_wr_nofa <- brms_fit_indices(b_null_wr_nofa);
  brms_compare_models(b_null_wr, b_null_wr_nofa, "[null with full raneffs]", "[null with language]"); # they are equivalent but the fixed effects estimates seem better for the reduced model
  
  # however, the intercept of the null model is really shitty:
  summary(b_null_wr_nofa);
  hdi(b_null_wr_nofa); # 1.78 [1.42, 2.17]
  rope(b_null_wr_nofa); # pROPE = 0.0%
  ...
  # Using the full model with the full random effects structure:  
  ...
  p <- mcmc_plot(b_L1_full, variable="^b_", regex=TRUE, type="intervals", prob=0.50, prob_outer=0.89, point_est="median") + 
    xlim(c(-4,6)) + 
    scale_y_discrete(labels=c("b_Intercept" = "Intercept", 
                              "b_Orderl_first" = "Order ([l] first)",
                              "b_SexM" = "Sex (male)",
                              "b_Age" = "Age",
                              "b_r_l_distinction_L1_fdistinct" = "r/l contrast",
                              "b_trill_real_L1_fyes" = "[r] is r-sound"),
                     limits=rev(c("b_Intercept", 
                                  "b_SexM",
                                  "b_Age",
                                  "b_Orderl_first",
                                  "b_trill_real_L1_fyes",
                                  "b_r_l_distinction_L1_fdistinct"))) +
    xlab('Coefficient') +
    ylab(NULL) +
    theme_timo + # Tweak cosmetics:
    theme(axis.title.x = element_text(margin = margin(t = 12, b = 0,
  ...
  ggsave(plot = p, filename = "./plots/full_model_web_coefficients.tif", width = 8, height = 6, compression="lzw", dpi=600);
  ...
  p <- newdata %>% 
    ...
    scale_shape(labels=c("yes"="[r] is r-sound", "no"="[r] is not r-sound")) + 
    theme_timo + 
  ...
  ggsave(plot = p, filename = "./plots/full_model_web_languages.tif", width = 12, height = 6, compression="lzw", dpi=600);
  
  
  ## And using the full model with the simplified random effects structure:
  b_web_for_plots <- brm(Match ~ 1 + 
                           Order + Sex + Age + 
                           r_l_distinction_L1_f + trill_real_L1_f +
                           (1 + Order + Sex + Age | Language),
                         data = web,
                         ...
                         sample_prior=TRUE,             # needed for hypotheses tests
                         seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=13));
  summary(b_web_for_plots); mcmc_plot(b_web_for_plots, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_web_for_plots, variable="^b_", regex=TRUE); # very decent
  
  # Extract the posteriors:
  posts <- posterior_samples(b_web_for_plots);
  posts <- select(posts, b_Intercept:b_trill_real_L1_fyes);
  ...
  p <- mcmc_plot(b_web_for_plots, variable="^b_", regex=TRUE, type="intervals", prob=0.50, prob_outer=0.89, point_est="median") + 
    #xlim(c(-4,6)) + 
    scale_y_discrete(labels=c("b_Intercept" = "Intercept", 
                              "b_Orderl_first" = "Order ([l] first)",
                              "b_SexM" = "Sex (male)",
  ...
  # Show and save:
  p;
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_coefficients.pdf", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_coefficients.jpg", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_coefficients.tif", width = 8, height = 6, compression="lzw", dpi=600);
  ...
  # Get predictions and append to dataframe:
  fit <- fitted(b_web_for_plots, newdata = newdata, re_formula = NULL, robust = TRUE);
  colnames(fit) = c('fit', 'se', 'lwr', 'upr')
  newdata <- cbind(newdata, fit)
  ...
  # Show and save:
  p
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_languages.pdf", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_languages.jpg", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_web_reducedraneeff_languages.tif", width = 12, height = 6, compression="lzw", dpi=600);
  ...
  if( .save_models )
  {
    ...
    saveRDS(b_L1_full, file="./models/web_regressions_L1_full_model.rds", compress="xz");
    saveRDS(b_web_for_plots, file="./models/web_regressions_L1_full_model_reducedraneeff.rds", compress="xz");
  ...
  web_regressions_L1_summaries <- list("glmer"=list("vif"   =performance::check_collinearity(m_vif), 
                                       ...
                                       "brms"=list("prior_pred_check"=list("plot_path"="./plots/web_L1_prior_predictive_checks.jpg"),
                                                   ...
                                                   "with_trill"       =list("hdi"=bayestestR::hdi(b_null_wr_nofa, ci=0.95), 
                                                                            "rope"=bayestestR::rope(b_null_wr_nofa, ci=0.95, ci_method="HDI", verbose=FALSE), 
                                                                            "fixef"=fixef(b_null_wr_nofa), "ranef"=ranef(b_null_wr_nofa), 
                                                                            "summary"=capture.output(summary(b_null_wr_nofa))),
                                                   ...
                                                   "full_for_plotting"=list("hdi"=bayestestR::hdi(b_L1_full, ci=0.95), 
                                                                            "rope"=bayestestR::rope(b_L1_full, ci=0.95, ci_method="HDI", verbose=FALSE), 
                                                                            "fixef"=fixef(b_L1_full), "ranef"=ranef(b_L1_full), 
                                                                            "summary"=capture.output(summary(b_L1_full))),
                                                   "full_for_plotting__reducedraneeff"=list("hdi"=bayestestR::hdi(b_web_for_plots, ci=0.95), 
                                                                                            "rope"=bayestestR::rope(b_web_for_plots, ci=0.95, ci_method="HDI", verbose=FALSE), 
                                                                                            "fixef"=fixef(b_web_for_plots), "ranef"=ranef(b_web_for_plots), 
                                                                                            "summary"=capture.output(summary(b_web_for_plots)))
                                       ));
...
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["Orderl_first", "Estimate"], tmpg$CI95["Orderl_first",1], tmpg$CI95["Orderl_first",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["Orderl_first", "Estimate"]), 100*lo2or(tmpg$CI95["Orderl_first",1]), 100*lo2or(tmpg$CI95["Orderl_first",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["Orderl_first", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["Orderl_first",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["Orderl_first",2]))` | `r scinot(tmpg$summary$coefficients["Orderl_first", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Orderl_first", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Orderl_first" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Orderl_first" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["Orderl_first", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Orderl_first" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["Orderl_first", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Orderl_first" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Orderl_first" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Orderl_first", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Orderl_first" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Orderl_first" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["Orderl_first", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Orderl_first" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["Orderl_first", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Orderl_first" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Orderl_first" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |

Thus, it is clear that *Order* has a **significant effect**, with "l first" *lowering* the probability of a *match* `r a=90; b=9; sprintf("from about %.0f%% to about %.0f%% (i.e., by about %.0f%%)", a, a-b, b)`.
...
|                 | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
|                 | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Intercept", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbs$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Intercept" ])` |  |
| *order* (*β*~M-F~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["SexM", "Estimate"], tmpg$CI95["SexM",1], tmpg$CI95["SexM",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["SexM", "Estimate"]), 100*lo2or(tmpg$CI95["SexM",1]), 100*lo2or(tmpg$CI95["SexM",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["SexM", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["SexM",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["SexM",2]))` | `r scinot(tmpg$summary$coefficients["SexM", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~M-F~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["SexM", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_SexM" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_SexM" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["SexM", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_SexM" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_SexM" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["SexM", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_SexM" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_SexM" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_SexM" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~M-F~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["SexM", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_SexM" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_SexM" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["SexM", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_SexM" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_SexM" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["SexM", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_SexM" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_SexM" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_SexM" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
...
|                 | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
|                 | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Intercept", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbs$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Intercept" ])` |  |
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["Age", "Estimate"], tmpg$CI95["Age",1], tmpg$CI95["Age",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["Age", "Estimate"]), 100*lo2or(tmpg$CI95["Age",1]), 100*lo2or(tmpg$CI95["Age",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["Age", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["Age",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["Age",2]))` | `r scinot(tmpg$summary$coefficients["Age", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Age", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["Age", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["Age", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Age" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Age", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Age" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Age" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["Age", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Age" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["Age", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Age" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Age" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
...
|                 | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
|                 | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Intercept", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbs$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Intercept" ])` |  |
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["r_l_distinction_L1_fdistinct", "Estimate"], tmpg$CI95["r_l_distinction_L1_fdistinct",1], tmpg$CI95["r_l_distinction_L1_fdistinct",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2or(tmpg$CI95["r_l_distinction_L1_fdistinct",1]), 100*lo2or(tmpg$CI95["r_l_distinction_L1_fdistinct",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["r_l_distinction_L1_fdistinct",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["r_l_distinction_L1_fdistinct",2]))` | `r scinot(tmpg$summary$coefficients["r_l_distinction_L1_fdistinct", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["r_l_distinction_L1_fdistinct", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_r_l_distinction_L1_fdistinct" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["r_l_distinction_L1_fdistinct", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["r_l_distinction_L1_fdistinct", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L1_fdistinct" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_r_l_distinction_L1_fdistinct" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
...
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["trill_real_L1_fyes", "Estimate"], tmpg$CI95["trill_real_L1_fyes",1], tmpg$CI95["trill_real_L1_fyes",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpg$CI95["trill_real_L1_fyes",1]), 100*lo2or(tmpg$CI95["trill_real_L1_fyes",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["trill_real_L1_fyes",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["trill_real_L1_fyes",2]))` | `r scinot(tmpg$summary$coefficients["trill_real_L1_fyes", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["trill_real_L1_fyes", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_trill_real_L1_fyes" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["trill_real_L1_fyes", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_trill_real_L1_fyes" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
| *order* (*β*~l_first-r_first~)   | B~nrs~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbi$fixef["trill_real_L1_fyes", "Estimate"], tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbi$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpbi$fixef["Intercept", "Estimate"], tmpbi$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_Intercept" ], tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_Intercept" ], tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbi$rope$ROPE_Percentage[ tmpbi$rope$Parameter == "b_trill_real_L1_fyes" ], tmpbi$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbi$cmp_0))` |
...
|                 | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
|                 | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Intercept", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbs$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Intercept" ])` |  |
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["r_l_distinction_L2_fdistinct", "Estimate"], tmpg$CI95["r_l_distinction_L2_fdistinct",1], tmpg$CI95["r_l_distinction_L2_fdistinct",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2or(tmpg$CI95["r_l_distinction_L2_fdistinct",1]), 100*lo2or(tmpg$CI95["r_l_distinction_L2_fdistinct",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["r_l_distinction_L2_fdistinct",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["r_l_distinction_L2_fdistinct",2]))` | `r scinot(tmpg$summary$coefficients["r_l_distinction_L2_fdistinct", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["r_l_distinction_L2_fdistinct", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_r_l_distinction_L2_fdistinct" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["r_l_distinction_L2_fdistinct", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["r_l_distinction_L2_fdistinct", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_r_l_distinction_L2_fdistinct" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_r_l_distinction_L2_fdistinct" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
...
|                 | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["Intercept", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbs$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_Intercept" ])` |  |
|                 | B~nrs~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbi$fixef["Intercept", "Estimate"], tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_Intercept" ], tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpbi$fixef["Intercept", "Estimate"]), 100*lo2p(tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpbi$rope$ROPE_Percentage[ tmpbi$rope$Parameter == "b_Intercept" ])` |  |
| *order* (*β*~l_first-r_first~)   | ML | `r sprintf("%.2f [%.2f, %.2f]", tmpg$summary$coefficients["trill_real_L2_fyes", "Estimate"], tmpg$CI95["trill_real_L2_fyes",1], tmpg$CI95["trill_real_L2_fyes",2])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpg$summary$coefficients["trill_real_L2_fyes", "Estimate"]), 100*lo2or(tmpg$CI95["trill_real_L2_fyes",1]), 100*lo2or(tmpg$CI95["trill_real_L2_fyes",2]), 100*lo2ps(tmpg$summary$coefficients["(Intercept)", "Estimate"], tmpg$summary$coefficients["trill_real_L2_fyes", "Estimate"]), 100*lo2ps(tmpg$CI95["(Intercept)",1], tmpg$CI95["trill_real_L2_fyes",1]), 100*lo2ps(tmpg$CI95["(Intercept)",2], tmpg$CI95["trill_real_L2_fyes",2]))` | `r scinot(tmpg$summary$coefficients["trill_real_L2_fyes", "Pr(>|z|)"])` | `r sprintf("**VS null: **χ^2^(%d)=%.2f, *p*=%s, ΔAIC=%.1f", tmpg$anova_0$Df[2], tmpg$anova_0$Chisq[2], scinot(tmpg$anova_0[2,"Pr(>Chisq)"]), tmpg$anova_0$AIC[2] - tmpg$anova_0$AIC[1])` |
| *order* (*β*~l_first-r_first~)   | B~fre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["trill_real_L2_fyes", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_trill_real_L2_fyes" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
| *order* (*β*~l_first-r_first~)   | B~sre~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbs$fixef["trill_real_L2_fyes", "Estimate"], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbs$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2or(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2or(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpbs$fixef["Intercept", "Estimate"], tmpbs$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2ps(tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_low[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_Intercept" ], tmpbs$hdi$CI_high[ tmpbs$hdi$Parameter == "b_trill_real_L2_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbs$rope$ROPE_Percentage[ tmpbs$rope$Parameter == "b_trill_real_L2_fyes" ], tmpbs$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbs$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbs$cmp_full))` |
| *order* (*β*~l_first-r_first~)   | B~nrs~  | `r sprintf("%.2f [%.2f, %.2f]", tmpbi$fixef["trill_real_L2_fyes", "Estimate"], tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ], tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpbi$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2or(tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2or(tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpbi$fixef["Intercept", "Estimate"], tmpbi$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2ps(tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_Intercept" ], tmpbi$hdi$CI_low[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_Intercept" ], tmpbi$hdi$CI_high[ tmpbi$hdi$Parameter == "b_trill_real_L2_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpbi$rope$ROPE_Percentage[ tmpbi$rope$Parameter == "b_trill_real_L2_fyes" ], tmpbi$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpbi$cmp_0))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~fre~: **", .print.model.comparison(b=tmpbi$cmp_full))` |
| 〃 | 〃 | 〃 | 〃 | 〃 | `r paste0("**VS B~sre~: **", .print.model.comparison(b=tmpbi$cmp_simple))` |
...
| *order* (*β*~l_first-r_first~)   | `r sprintf("%.2f [%.2f, %.2f]", tmpor$fixef["Orderl_first", "Estimate"], tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_Orderl_first" ], tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_Orderl_first" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpor$fixef["Orderl_first", "Estimate"]), 100*lo2or(tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_Orderl_first" ]), 100*lo2or(tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpor$fixef["Intercept", "Estimate"], tmpor$fixef["Orderl_first", "Estimate"]), 100*lo2ps(tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_Intercept" ], tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_Intercept" ], tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_Orderl_first" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpor$rope$ROPE_Percentage[ tmpor$rope$Parameter == "b_Orderl_first" ], tmpor$hypotheses$hypothesis[1, "Post.Prob"])` |
| *[r]* (*β*~yes-no~)   | `r sprintf("%.2f [%.2f, %.2f]", tmpor$fixef["trill_real_L1_fyes", "Estimate"], tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpor$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpor$fixef["Intercept", "Estimate"], tmpor$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_Intercept" ], tmpor$hdi$CI_low[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpor$hdi$CI_high[ tmpor$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpor$rope$ROPE_Percentage[ tmpor$rope$Parameter == "b_trill_real_L1_fyes" ], tmpor$hypotheses$hypothesis[2, "Post.Prob"])` |
...
| *[r]* (*β*~yes-no~)   | `r sprintf("%.2f [%.2f, %.2f]", tmpo$fixef["trill_real_L1_fyes", "Estimate"], tmpo$hdi$CI_low[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpo$hdi$CI_high[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpo$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpo$hdi$CI_low[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpo$hdi$CI_high[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpo$fixef["Intercept", "Estimate"], tmpo$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpo$hdi$CI_low[ tmpo$hdi$Parameter == "b_Intercept" ], tmpo$hdi$CI_low[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpo$hdi$CI_high[ tmpo$hdi$Parameter == "b_Intercept" ], tmpo$hdi$CI_high[ tmpo$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpo$rope$ROPE_Percentage[ tmpo$rope$Parameter == "b_trill_real_L1_fyes" ], tmpo$hypotheses$hypothesis[1, "Post.Prob"])` |
...
Nevertheless, even if we were to accept that the *presence of [r]* in L1 would affect the probability of a *match*, the presence of this sound in L1 would *lower* the probability of a *match* `r a=95; b=8; sprintf("from about %.0f%% to about %.0f%% (i.e., by about %.0f%%)", a, a-b, b)`..
...
With these, the full model Match ~ 1 + Order + Sex + Age + (1 + Order + Sex + Age | Language) + (1 + Order + Sex + Age | Family) + (1 + Order + Sex + Age | Autotyp_Area) is not much better than the null model containing only the intercept, as expected; however, the full random effects structure is not necessary and reduces to (1 | Language).
With these, the intercept is positive (>50%) and its 95%HDI excludes 0 (50%): `r tmp <- web_regressions_L1_summaries$brms$with_trill; sprintf("%.2f 95%%HDI [%.2f, %.2f]", tmp$fixef["Intercept", "Estimate"], tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ], tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ])` (as probability: `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmp$fixef["Intercept", "Estimate"]), 100*lo2p(tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ]))`), `r sprintf("pROPE=%.1f%%", tmp$rope$ROPE_Percentage[ tmp$rope$Parameter == "b_Intercept" ])`.
...
With these, the full model Match ~ 1 + Order + Sex + Age + r_l_distinction_L1_f + (1 + Order + Sex + Age | Language) + (1 + Order + Sex + Age | Family) + (1 + Order + Sex + Age | Autotyp_Area) simplifies to the model only with *Order* as fixed effect and *Languages* as random effect, but the random slope of *Order* is necessary: Match ~ 1 + Order + (1 + Order | Language).
...
| *order* (*β*~l_first-r_first~)   | `r sprintf("%.2f [%.2f, %.2f]", tmp$fixef["Orderl_first", "Estimate"], tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Orderl_first" ], tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Orderl_first" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmp$fixef["Orderl_first", "Estimate"]), 100*lo2or(tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Orderl_first" ]), 100*lo2or(tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmp$fixef["Intercept", "Estimate"], tmp$fixef["Orderl_first", "Estimate"]), 100*lo2ps(tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ], tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Orderl_first" ]), 100*lo2ps(tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ], tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Orderl_first" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmp$rope$ROPE_Percentage[ tmp$rope$Parameter == "b_Orderl_first" ], tmp$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmp$cmp_0))` |
...
Interestingly, the presence or not of [r] in the L2(s) does not seem to matter.
...
<center>![Full model: the fixed effects showing the median, 50% and 89% quantiles of the posterior distribution.](./plots/full_model_web_coefficients.jpg){width="6in"}</center>
...
And using the *full model* but with a reduced random structure (i.e., Match ~ 1 + Order + Sex + Age + r_l_distinction_L1_f + trill_real_L1_f + (1 + Order + Sex + Age | Language)):

<center>![Full model: the fixed effects showing the median, 50% and 89% quantiles of the posterior distribution.](./plots/full_model_web_reducedraneeff_coefficients.jpg){width="6in"}</center>

<center>![Full model: the distribution of the proportion of *matches* per language.](./plots/full_model_web_reducedraneeff_languages.jpg){width="10in"}</center>

Looking separately at the cases where [r] is present in the L1(s) and where it is not shows that, in the first case the association between [r] and the ragged line is weaker and there is no effect of the order of presentation, but that in the second case the association is much stronger and order does have an effect in the sense that presenting "l" first reduces the probability of a match from ~95% to about 89%.
    ...
    !(.save_models & file.exists("./models/field_regressions_models.rds")) )
{
  ...
  ## Let's investigate what happens if we analyze the languages with and without [r] in L1 separately:
  # with [r]:
  field_wr <- field[ field$trill_real_L1_f == "yes", ]; # two languages from two families and areas -> Language should be enough as random effect
  # constant: r_l_distinction_L1_f trill_occ_L1 r_l_distinction_L2 trill_real_L2 trill_occ_L2
  ...
  b_full_wr <- brm(Match ~ 1 + 
                   ...
                   seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=13));
  summary(b_full_wr); mcmc_plot(b_full_wr, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_full_wr, variable="^b_", regex=TRUE); # very decent
  brms::hypothesis(b_full_wr, c("Sexm = 0",                                              # p=0.51
                                "Age = 0",                                               # p=0.64
                                NULL));
  bayestestR::hdi(b_full_wr, ci=0.95); # (Intercept) | [-130.23, 121.72]; Sexm        | [  -6.16,   6.35]; Age         | [  -4.01,   4.60]
  bayestestR::rope(b_full_wr, ci=0.95, ci_method="HDI"); # Intercept |      0.30 %; Sexm      |      5.90 %; Age       |      9.72 %
  b_full_wr <- brms_fit_indices(b_full_wr);
  brms_compare_models(b_null_wr, b_full_wr, "[null]", "[full]"); # equivalent
  # -> none matters

  # without [r]:
  field_nr <- field[ field$trill_real_L1_f == "no", ]; # 4 languages from two families and areas -> Language and Family should be enough as random effect
  ...
  b_full_nr <- brm(Match ~ 1 + 
                     Sex + Age + r_l_distinction_L1_f + trill_occ_L1_f +
                     (1 + Sex + Age | Language) + # r_l_distinction_L1_f & trill_occ_L1_f do not conceptually vary by language
                     (1 + Sex + Age | Family),
                   data = field_nr,
                   family=bernoulli(link='logit'),
                   prior=c(brms::set_prior("student_t(5, 0, 2.5)", class="b"), 
                   ...
                   seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=13));
  summary(b_full_nr); mcmc_plot(b_full_nr, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_full_nr, variable="^b_", regex=TRUE); # very decent
  brms::hypothesis(b_full_nr, c("r_l_distinction_L1_fdistinct = 0",                      # p=0.50
                                "trill_occ_L1_fyes = 0",                                 # p=0.50
                                "Sexm = 0",                                              # p=0.60
                                "Age = 0",                                               # p=0.88
                                NULL));
  bayestestR::hdi(b_full_nr, ci=0.95); # (Intercept) | [-5.21, 19.32]; Sexm | [-3.73,  4.48]; Age | [-0.63,  0.15]; r_l_distinction_L1_fdistinct | [-6.91,  5.98]; trill_occ_L1_fyes | [-6.85,  5.62]
  bayestestR::rope(b_full_nr, ci=0.95, ci_method="HDI"); # Intercept |      0.82 %; Sexm |      8.50 %; Age |     45.56 %; r_l_distinction_L1_fdistinct |      5.64 %; trill_occ_L1_fyes |      5.81 %
  b_full_nr <- brms_fit_indices(b_full_nr);
  brms_compare_models(b_null_nr, b_full_nr, "[null]", "[full]"); # null is better than full!
  # -> we can remove age and sex:
  
  b_red_nr <- brm(Match ~ 1 + 
                    r_l_distinction_L1_f + trill_occ_L1_f +
                    (1 | Language) + # r_l_distinction_L1_f & trill_occ_L1_f do not conceptually vary by language
                    (1 | Family),
                  data = field_nr,
                  family=bernoulli(link='logit'),
                  prior=c(brms::set_prior("student_t(5, 0, 2.5)", class="b")),
                  ...
                  sample_prior=TRUE,             # needed for hypotheses tests
                  seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=13));
  summary(b_red_nr); mcmc_plot(b_red_nr, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_red_nr, variable="^b_", regex=TRUE); # very decent
  brms::hypothesis(b_red_nr, c("r_l_distinction_L1_fdistinct = 0",                      # p=0.50
                               "trill_occ_L1_fyes = 0",                                 # p=0.50
                               NULL));
  bayestestR::hdi(b_red_nr, ci=0.95); # (Intercept) | [-2.10, 12.54]; r_l_distinction_L1_fdistinct | [-6.79,  4.31]; trill_occ_L1_fyes | [-6.80,  4.17]
  bayestestR::rope(b_red_nr, ci=0.95, ci_method="HDI"); # Intercept |      1.16 %; r_l_distinction_L1_fdistinct |      5.79 %; trill_occ_L1_fyes |      5.81 %
  b_red_nr <- brms_fit_indices(b_red_nr);
  brms_compare_models(b_null_nr, b_red_nr, "[null]", "[reduced]"); # equivalent
  brms_compare_models(b_full_nr, b_red_nr, "[full]", "[reduced]"); # reduced is better
  
  ## -> so, both for those with and without [r], no factor seems to play any role
  ...
  summary(b_null_nr_noaf); mcmc_plot(b_null_nr_noaf, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_null_nr_noaf, variable="^b_", regex=TRUE); # very decent
  b_null_nr_noaf <- brms_fit_indices(b_null_nr_noaf);
  brms_compare_models(b_null_nr, b_null_nr_noaf, "[null full random]", "[null only language]"); # equivalent
  
  ##    and the intercept (of the null model) are not impressive
  ...
  b_L1_full <- brm(Match ~ 1 + 
                     Sex + Age + 
                     r_l_distinction_L1_f + trill_real_L1_f + trill_occ_L1_f +
                     (1 + Sex + Age | Language) +
                     (1 + Sex + Age | Family) +
                     (1 + Sex + Age | Autotyp_Area),
                   data = field,
                   family=bernoulli(link='logit'),
                   prior=c(brms::set_prior("student_t(5, 0, 2.5)", class="b"), 
                           brms::set_prior("lkj(2)", class="cor")),
                   save_pars=save_pars(all=TRUE), # needed for Bayes factors
                   sample_prior=TRUE,             # needed for hypotheses tests
                   seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=15));
  summary(b_L1_full); mcmc_plot(b_L1_full, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_L1_full, variable="^b_", regex=TRUE); # very decent
  brms::hypothesis(b_L1_full, c("Sexm = 0",                                    # p=0.57
                                "Age = 0",                                     # p=0.87
                                "r_l_distinction_L1_fdistinct = 0",            # p=0.50
                                "trill_real_L1_fyes = 0",                      # p=0.50
                                "trill_occ_L1_fyes = 0"));                     # p=0.50
  # posterior predictive checks
  plot_prefix <- "./plots/field_L1_full"; b <- b_L1_full;
  mcmc_plot(b, type="trace", variable="^b_", regex=TRUE); ggsave(paste0(plot_prefix,"_mcmctrace.jpg"), device="jpeg", width=7, height=6, units="in", quality=85);
  mcmc_plot(b, variable="^b_", regex=TRUE); ggsave(paste0(plot_prefix,"_mcmcestim.jpg"), device="jpeg", width=7, height=4, units="in", quality=85);
  pp_check(b, ndraws=100) + xlab('p(match)') + ggtitle('Posterior predictive density overlay'); ggsave(paste0(plot_prefix,"_ppchecks_densoverlay.jpg"), device="jpeg", width=6, height=4, units="in", quality=85);
  g <- arrangeGrob(pp_check(b, type='stat', sta ='min') + xlab('p(match)') + ggtitle('Posterior predictive distribution of minimum values'),
                   ...
                   ncol=1); # seems fine (the observed, y, does fall in the predicted distributions, y_{rep} for the max, mean and min)
  as_ggplot(g); ggsave(paste0(plot_prefix,"_ppchecks_min_mean_max.jpg"), plot=g, device="jpeg", width=6, height=4*3, units="in", quality=85);
  plot_model(b, type="pred", terms=c("trill_real_L1_f", "r_l_distinction_L1_f", "trill_occ_L1_f")); ggsave(paste0(plot_prefix,"_pred.jpg"), device="jpeg", width=4, height=4, units="in", quality=85);
  
  # Extract the posteriors:
  posts <- posterior_samples(b_L1_full);
  posts <- select(posts, b_Intercept:b_trill_occ_L1_fyes);
  
  # Make the posteriors into long format for plotting:
  posts <- pivot_longer(posts,
                        cols = b_Intercept:b_trill_occ_L1_fyes,
                        names_to = 'coefficient',
                        values_to = 'posterior');
  ...
  posts <- posts %>%
    mutate(coefficient = ifelse(coefficient == 'b_Intercept',                     'Intercept', coefficient),
           ...
           coefficient = ifelse(coefficient == 'b_r_l_distinction_L1_fdistinct',  'r/l distinction', coefficient),
           coefficient = ifelse(coefficient == 'b_trill_real_L1_fyes',            '[r] present', coefficient),
           coefficient = ifelse(coefficient == 'b_trill_occ_L1_fyes',             '[r] as allophone', coefficient),
           coefficient = factor(coefficient, levels = rev(c('Intercept', 'Order', 'Sex', 'Age', 'r/l distinction', '[r] present', '[r] as allophone'))));
  ...
  p <- mcmc_plot(b_L1_full, variable="^b_", regex=TRUE, type="intervals", prob=0.50, prob_outer=0.89, point_est="median") + 
    ...
    scale_y_discrete(labels=c("b_Intercept" = "Intercept", 
                              ...
                              "b_r_l_distinction_L1_fdistinct" = "r/l contrast",
                              "b_trill_real_L1_fyes" = "[r] is r-sound",
                              "b_trill_occ_L1_fyes" = "[r] as allophone"),
                     limits=rev(c("b_Intercept", 
                                  "b_Sexm",
                                  "b_Age",
                                  "b_trill_real_L1_fyes",
  ...
  # Show and save:
  p;
  ggsave(plot = p, filename = "./plots/full_model_field_coefficients.pdf", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_coefficients.jpg", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_coefficients.tif", width = 8, height = 6, compression="lzw", dpi=600);

  
  ## Combine the predictions for each language with the averages of each language:
  # Setup data frame with predictors to get predictions for:
  newdata <- data.frame(Language = unique(field$Language));
  newdata$Sex <- "f";
  newdata$Age <- mean(field$Age);
  newdata$trill_real_L1_f <- field[match(newdata$Language, field$Language), ]$trill_real_L1_f
  newdata$r_l_distinction_L1_f <- field[match(newdata$Language, field$Language), ]$r_l_distinction_L1_f
  newdata$trill_occ_L1_f <- field[match(newdata$Language, field$Language), ]$trill_occ_L1_f
  newdata$Family <- field[match(newdata$Language, field$Language), ]$Family
  newdata$Autotyp_Area <- field[match(newdata$Language, field$Language), ]$Autotyp_Area
  ...
  # Finally, add the averages to the plot:
  newdata$avg <- field_avg[match(newdata$Language, field_avg$Language), ]$M
  
  # Match language names into there and order:
  newdata$Language <- field[match(newdata$Language, field$Language), ]$Name
  newdata <- mutate(newdata, Language = factor(Language, levels = newdata$Language))
  ...
  p <- newdata %>% 
    ...
    geom_point(size = 5) +
    geom_hline(yintercept = 0.5, linetype = 2, size = 1.5, col = 'grey') + 
    geom_point(aes(y = avg), col = 'black', shape = 23, size = 7, stroke = 1.0, alpha = 0.75)  +
    labs(x = '', y = 'Proportion of congruent responses') + # Axis labels:
    #ggtitle('Posterior medians and descriptive averages of congruent responses\nby language and R/L contrast (color) and [r] as primary rhotic in L1 (shape)') +
    scale_color_manual(values = c("same"=colorBlindBlack8[2], "distinct"=colorBlindBlack8[3]), labels=c("same"="no r/l contrast", "distinct"="r/l contrast")) + # Tweak cosmetics:
    scale_shape(labels=c("yes"="[r] is r-sound", "no"="[r] is not r-sound")) + 
  ...
  # Show and save:
  p
  ggsave(plot = p, filename = "./plots/full_model_field_languages.pdf", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_languages.jpg", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_languages.tif", width = 12, height = 6, compression="lzw", dpi=600);
  
  
  ## Using the full model wth a reduced random effects structure:
  b_field_for_plots <- brm(Match ~ 1 + 
                             Sex + Age + 
                             r_l_distinction_L1_f + trill_real_L1_f + trill_occ_L1_f +
                             (1 + Sex + Age | Language),
                           data = field,
                           ...
                           sample_prior=TRUE,             # needed for hypotheses tests
                           seed=998, cores=brms_ncores, iter=10000, warmup=4000, thin=2, control=list(adapt_delta=0.9999, max_treedepth=15));
  summary(b_field_for_plots); mcmc_plot(b_field_for_plots, type="trace", variable="^b_", regex=TRUE); mcmc_plot(b_field_for_plots, variable="^b_", regex=TRUE); # very decent

  # Extract the posteriors:
  posts <- posterior_samples(b_field_for_plots);
  posts <- select(posts, b_Intercept:b_trill_occ_L1_fyes);
  ...
  p <- mcmc_plot(b_field_for_plots, variable="^b_", regex=TRUE, type="intervals", prob=0.50, prob_outer=0.89, point_est="median") + 
    #xlim(c(-4,6)) + 
    scale_y_discrete(labels=c("b_Intercept" = "Intercept", 
                              "b_Sexm" = "Sex (male)",
                              "b_Age" = "Age",
  ...
  # Show and save:
  p;
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_coefficients.pdf", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_coefficients.jpg", width = 8, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_coefficients.tif", width = 8, height = 6, compression="lzw", dpi=600);
  ...
  # Get predictions and append to dataframe:
  fit <- fitted(b_field_for_plots, newdata = newdata, re_formula = NULL, robust = TRUE);
  colnames(fit) = c('fit', 'se', 'lwr', 'upr')
  newdata <- cbind(newdata, fit)
  ...
  # Show and save:
  p
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_languages.pdf", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_languages.jpg", width = 12, height = 6);
  ggsave(plot = p, filename = "./plots/full_model_field_reducedraneeff_languages.tif", width = 12, height = 6, compression="lzw", dpi=600);
  ...
  if( .save_models )
  {
    ...
    field_regressions_models_separate <- list("brms"=list("with_r"=b_null_wr,
                                                          "without_r"=b_null_nr_noaf));
    save(field_regressions_models_separate, file="./models/field_regressions_models_separate.rds", compress="xz", compression_level=9);
    saveRDS(b_L1_full, file="./models/field_regressions_L1_full_model.rds", compress="xz");
    saveRDS(b_field_for_plots, file="./models/field_regressions_L1_full_model_reducedraneeff.rds", compress="xz");
  }
  # save the summaries (much less disk space but enough info to understand the results):
  field_regressions_summaries <- list("brms"=list("prior_pred_check"=list("plot_path"="./plots/field_prior_predictive_checks.jpg"),
                                                  ...
                                                  "full_for_plotting"=list("hdi"=bayestestR::hdi(b_L1_full, ci=0.95), 
                                                                            ...
                                                                            "fixef"=fixef(b_L1_full), "ranef"=ranef(b_L1_full), 
                                                                            "summary"=capture.output(summary(b_L1_full))),
                                                  "full_for_plotting_reducedraneeff"=list("hdi"=bayestestR::hdi(b_field_for_plots, ci=0.95), 
                                                                                          "rope"=bayestestR::rope(b_field_for_plots, ci=0.95, ci_method="HDI", verbose=FALSE), 
                                                                                          "fixef"=fixef(b_field_for_plots), "ranef"=ranef(b_field_for_plots), 
                                                                                          "summary"=capture.output(summary(b_field_for_plots)))));
  save(field_regressions_summaries, file="./models/field_regressions_summaries.rds", compress="xz", compression_level=9);
  field_regressions_summaries_separate <- list("brms"=list("with_r" =list("hdi"=bayestestR::hdi(b_null_wr, ci=0.95),
                                                           ...
                                                           "without_r" =list("hdi"=bayestestR::hdi(b_null_nr_noaf, ci=0.95),
                                                                             "rope"=bayestestR::rope(b_null_nr_noaf, ci=0.95, ci_method="HDI", verbose=FALSE),
                                                                             "fixef"=fixef(b_null_nr_noaf), "ranef"=ranef(b_null_nr_noaf),
                                                                             "summary"=capture.output(summary(b_null_nr_noaf)))));
  save(field_regressions_summaries_separate, file="./models/field_regressions_summaries_separate.rds", compress="xz", compression_level=9);
} else
{
  ...
  #if( .save_models && file.exists("./models/field_regressions_models_separate.rds") ) load("./models/field_regressions_models_separate.rds") else field_regressions_models_separate <- NULL;
  ...
  if( file.exists("./models/field_regressions_summaries_separate.rds") ) load("./models/field_regressions_summaries_separate.rds") else field_regressions_summaries_separate <- NULL; 
}
```
...
|---------:|-------:|---------------------:|----------------:|----------:|-----------------:|
| intercept (*α*) | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
| *age* (*β*)     | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Age", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["Age", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["Age", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Age" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Age" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Age" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
...
|---------:|-------:|---------------------:|----------------:|----------:|-----------------:|
| intercept (*α*) | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
| *[r]* (*β*~yes-no~) | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["trill_real_L1_fyes", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["trill_real_L1_fyes", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L1_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_trill_real_L1_fyes" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
...
|---------:|-------:|---------------------:|----------------:|----------:|-----------------:|
| intercept (*α*) | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["Intercept", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmpb$fixef["Intercept", "Estimate"]), 100*lo2p(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ]))` | `r sprintf("%.1f%%", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_Intercept" ])` |  |
| *[r]* (*β*~yes-no~) | B  | `r sprintf("%.2f [%.2f, %.2f]", tmpb$fixef["trill_real_L2_fyes", "Estimate"], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ])` | `r sprintf("%.1f%% [%.1f%%, %.1f%%]; %.1f%% [%.1f%%, %.1f%%]", 100*lo2or(tmpb$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2or(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2or(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpb$fixef["Intercept", "Estimate"], tmpb$fixef["trill_real_L2_fyes", "Estimate"]), 100*lo2ps(tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_low[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]), 100*lo2ps(tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_Intercept" ], tmpb$hdi$CI_high[ tmpb$hdi$Parameter == "b_trill_real_L2_fyes" ]))` | `r sprintf("pROPE=%.1f%%, *p*(*β*=0)=%.2f", tmpb$rope$ROPE_Percentage[ tmpb$rope$Parameter == "b_trill_real_L2_fyes" ], tmpb$hypotheses$hypothesis[1, "Post.Prob"])` | `r paste0("**VS null: **", .print.model.comparison(b=tmpb$cmp_0))` |
...
There are `r sum(field$trill_real_L1_f == "yes", na.rm=TRUE)` observations with [r] in L1 in the data.
The full model is equivalent with the null model containing only the intercept.
In this case, the intercept is positive (>50%) but its 95%HDI does include 0 (50%): `r tmp <- field_regressions_summaries_separate$brms$with_r; sprintf("%.2f 95%%HDI [%.2f, %.2f]", tmp$fixef["Intercept", "Estimate"], tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ], tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ])` (as probability: `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmp$fixef["Intercept", "Estimate"]), 100*lo2p(tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ]))`), `r sprintf("pROPE=%.1f%%", tmp$rope$ROPE_Percentage[ tmp$rope$Parameter == "b_Intercept" ])`, but the sample is very small.

#### Languages with*out* [r]

There are `r sum(field$trill_real_L1_f == "no", na.rm=TRUE)` observations with [r] in L1 in the data.
The full model is equivalent with the null model containing only the intercept.
In this case, the intercept is positive (>50%) but its 95%HDI does include 0 (50%): `r tmp <- field_regressions_summaries_separate$brms$without_r; sprintf("%.2f 95%%HDI [%.2f, %.2f]", tmp$fixef["Intercept", "Estimate"], tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ], tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ])` (as probability: `r sprintf("%.1f%% [%.1f%%, %.1f%%]", 100*lo2p(tmp$fixef["Intercept", "Estimate"]), 100*lo2p(tmp$hdi$CI_low[ tmp$hdi$Parameter == "b_Intercept" ]), 100*lo2p(tmp$hdi$CI_high[ tmp$hdi$Parameter == "b_Intercept" ]))`), `r sprintf("pROPE=%.1f%%", tmp$rope$ROPE_Percentage[ tmp$rope$Parameter == "b_Intercept" ])`, again the sample size is small.
...
<center>![Full model: the fixed effects showing the median, 50% and 89% quantiles of the posterior distribution.](./plots/full_model_field_coefficients.jpg){width="6in"}</center>

<center>![Full model: the distribution of the proportion of *matches* per language.](./plots/full_model_field_languages.jpg){width="10in"}</center>

And using the *full model* with the refuced random effects strucutre (Match ~ 1 + Sex + Age + r_l_distinction_L1_f + trill_real_L1_f + trill_occ_L1_f + (1 + Sex + Age | Language)):

<center>![Full model: the fixed effects showing the median, 50% and 89% quantiles of the posterior distribution.](./plots/full_model_field_reducedraneeff_coefficients.jpg){width="6in"}</center>

<center>![Full model: the distribution of the proportion of *matches* per language.](./plots/full_model_field_reducedraneeff_languages.jpg){width="10in"}</center>
...
## A.I: Online experiment

### A.I.1: The probability of a perfect match in the absence of any predictors 
...
```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$order_simple$summary, collapse="\n"));
```

### A.I.3: Sex

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$sex_simple$summary, collapse="\n"));
```

### A.I.4: Age

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$age_simple$summary, collapse="\n"));
```

### A.I.5: R/L distinction in L1?

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$rl_simple$summary, collapse="\n"));
```

### A.I.6: [r] in L1?

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$trill_simple_nrs$summary, collapse="\n"));
```

### A.I.7: R/L distinction in L2?

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$rl2_simple$summary, collapse="\n"));
```

### A.I.8: [r] in L2?

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$trill2_simple_nrs$summary, collapse="\n"));
```

### A.I.9: Order + [r] in L1

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$order_trill_LFA$summary, collapse="\n"));
```

### A.I.10: Languages with [r]

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$with_trill$summary, collapse="\n"));
```

### A.I.11: Languages without [r]

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$without_trill$summary, collapse="\n"));
```

### A.I.12: Full model used for plotting (full random effects structure)

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$full_for_plotting$summary, collapse="\n"));
```

### A.I.13: Full model used for plotting (reduced random effects structure)

```{r echo=FALSE}
cat(paste0(web_regressions_L1_summaries$brms$full_for_plotting__reducedraneeff$summary, collapse="\n"));
```


## A.I: Field experiment
...
```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$sex$summary, collapse="\n"));
```

### A.I.3: Age

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$age$summary, collapse="\n"));
```

### A.I.4: R/L distinction in L1?

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$rl$summary, collapse="\n"));
```

### A.I.5: [r] in L1?

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$trill$summary, collapse="\n"));
```

### A.I.6: R/L distinction in L2?

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$rl2$summary, collapse="\n"));
```

### A.I.7: [r] in L2?

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$trill2$summary, collapse="\n"));
```

### A.I.8: Languages with [r]

```{r echo=FALSE}
cat(paste0(field_regressions_summaries_separate$brms$with_r$summary, collapse="\n"));
```

### A.I.9: Languages without [r]

```{r echo=FALSE}
cat(paste0(field_regressions_summaries_separate$brms$without_r$summary, collapse="\n"));
```

### A.I.10: Full model used for plotting (full random effects structure)

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$full_for_plotting$summary, collapse="\n"));
```

### A.I.11: Full model used for plotting (reduced random effects structure)

```{r echo=FALSE}
cat(paste0(field_regressions_summaries$brms$full_for_plotting_reducedraneeff$summary, collapse="\n"));
```
